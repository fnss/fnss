 ...
 * **adapters**: contains functions for exporting FNSS objects to target simulators or emulators.
   Currently, this package includes functions for exporting FNSS objects to `Mininet <http://www.mininet.org>`_, `ns-2 <http://www.isi.edu/nsnam/ns/>`_, and `AutoNetKit <http://www.autonetkit.org>`_.
 * **topologies**: contains all functions and classes for parsing or synthenically generating a network topology. 
   ...
   to XML files is needed to make topology available for hte Java and C++ API and the `ns-3 <http://www.nsnam.org/>`_ adapter.
