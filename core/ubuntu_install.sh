...
#
# If this script is run on Ubuntu 14.04, it may be possible to
# also install all dependencies for running FNSS with Python 3.
# The code doing this is commented out by default. 
...
sudo apt-get -y -q install python python-pip python-setuptools python-numpy python-scipy python-networkx python-mako python-nose

# Install other optional dependencies not available on Ubuntu package repository
sudo pip install -U sphinx numpydoc

# Install FNSS core library
sudo pip install -U fnss

# Install C++ and Java dependencies
sudo apt-get -y -q install clang doxygen ant
...
#
# Uncomment the following code to install Python 3 dependencies
#
# VERSION=$(lsb_release -r 2>/dev/null | awk '{print $2}')
# type "lsb_release" > /dev/null
# if [ $? -eq 0 ]; then
# 	if [ $VERSION = "14.04" ]; then
# 		sudo apt-get -y -q install python3-pip python3-setuptools python3-numpy python3-scipy python3-mako python3-nose
